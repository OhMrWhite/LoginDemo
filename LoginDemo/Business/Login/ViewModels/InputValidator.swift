...
extension InputValidator {
    ...
    class func validateUserName(_ username: String) -> Result {
        ...
        if !username.validateEmail() {
            return Result.failure(message: "输入的Email格式错误".localized)
        }
        //email可用
        return Result.success(message: "账号可用".localized)
    }
    //判断密码
    class func validatePassword(_ password: String) -> Result {
        //判断密码长度
        if !isValidPassword(password) {
            return Result.failure(message: "输入的密码长度不足".localized)
        }
        //密码可用
        return Result.success(message: "密码可用".localized)
    }
    //判断用户名与密码
    class func validateLogin(_ email: Result, _ password : Result) -> Result {
        if email.isValid && password.isValid{
            return Result.success(message: "登录成功".localized)
        }
        return Result.failure(message: "登录失败".localized)
    }
}
